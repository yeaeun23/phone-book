...
public partial class Index : Page
{
    ...
    int ROWS_PER_PAGE = Convert.ToInt32(ReadValue("PAGINATION", "ROWS_PER_PAGE"));
    int PAGE_PER_PAGE = Convert.ToInt32(ReadValue("PAGINATION", "PAGE_PER_PAGE"));
    ...
    public static string ReadValue(string section, string key)
    {
        ...
        try
        {
            ...
            //GetPrivateProfileString(section, key, string.Empty, tmp, 255, @".\NewsContact.ini");
            return tmp.ToString();
        }
        catch (Exception)
        {
...
from [t_newsperson] a where a.b_useflag = 1 ";
        ...
        {
            if (selected_kindcode == 0)
                sql += string.Format(@"and replace(upper(v_name), ' ', '') like '%{0}%' or replace(upper(v_name_en), ' ', '') like '%{0}%' or replace(upper(v_tel1), ' ', '') like '%{0}%' or replace(upper(v_tel2), ' ', '') like '%{0}%' or replace(upper(v_email1), ' ', '') like '%{0}%' or replace(upper(v_email2), ' ', '') like '%{0}%' or replace(upper(convert(varchar(max), t_etc)), ' ', '') like '%{0}%' ", param_search);
            else
                sql += string.Format(@"and n_kindcode = {0} and (replace(upper(v_name), ' ', '') like '%{1}%' or replace(upper(v_name_en), ' ', '') like '%{1}%' or replace(upper(v_tel1), ' ', '') like '%{1}%' or replace(upper(v_tel2), ' ', '') like '%{1}%' or replace(upper(v_email1), ' ', '') like '%{1}%' or replace(upper(v_email2), ' ', '') like '%{1}%' or replace(upper(convert(varchar(max), t_etc)), ' ', '') like '%{1}%') ", selected_kindcode, param_search);
        }

        if (param_name != "")
            sql += @"order by v_name " + param_name + ", n_personid desc ";
        else if (param_no != "")
            sql += @"order by n_personid " + param_no + " ";
        else
            sql += @"order by v_name asc, n_personid desc ";
    ...
    private void SetPage()
    {
        ...
        {
            if (selected_kindcode == 0)
                dt = Util.ExeQuery(new SqlCommand(string.Format(@"select count(*) from [t_newsperson] where b_useflag = 1 and replace(upper(v_name), ' ', '') like '%{0}%' or replace(upper(v_name_en), ' ', '') like '%{0}%' or replace(upper(v_tel1), ' ', '') like '%{0}%' or replace(upper(v_tel2), ' ', '') like '%{0}%' or replace(upper(v_email1), ' ', '') like '%{0}%' or replace(upper(v_email2), ' ', '') like '%{0}%' or replace(upper(convert(varchar(max), t_etc)), ' ', '') like '%{0}%'", param_search)), "SELECT");
            else
                dt = Util.ExeQuery(new SqlCommand(string.Format(@"select count(*) from [t_newsperson] where b_useflag = 1 and n_kindcode = {0} and (replace(upper(v_name), ' ', '') like '%{1}%' or replace(upper(v_name_en), ' ', '') like '%{1}%' or replace(upper(v_tel1), ' ', '') like '%{1}%' or replace(upper(v_tel2), ' ', '') like '%{1}%' or replace(upper(v_email1), ' ', '') like '%{1}%' or replace(upper(v_email2), ' ', '') like '%{1}%' or replace(upper(convert(varchar(max), t_etc)), ' ', '') like '%{1}%')", selected_kindcode, param_search)), "SELECT");
        }
